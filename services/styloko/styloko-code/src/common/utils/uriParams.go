...
func GetPathParams(io orchestrator.WorkFlowData) (params []string, err error) {
	...
	if !ok || appHTTPReq == nil {
		logger.Error("Bad request. Fail at GetPathParams.")
	...
	if len(allSubDirs) <= 3 {
		if len(allSubDirs) == 2 {
			logger.Info("No path params found")
			return nil, errors.New(PathNoParams)
		}
		logger.Info("Invalid path provided.")
		return nil, errors.New(PathInvalid)
	}
...
func GetQueryParams(io orchestrator.WorkFlowData, key string) (string, bool) {
	httpReq, _ := io.IOData.Get(florest_constants.REQUEST)
	appHTTPReq, ok := httpReq.(*utilhttp.Request)
	if !ok || appHTTPReq == nil {
...
func GetRawQueryMap(io orchestrator.WorkFlowData) (map[string][]string, error) {
	...
	qParams := map[string][]string(rawReq.URL.Query())
	return qParams, nil
}
...
// GetQueryParam returns query params based on key
func GetQueryParam(httpReq *utilhttp.Request, key string) (interface{}, error) {
	rawReq := httpReq.OriginalRequest
	q := rawReq.FormValue(key)
	if q == "" {
		return nil, errors.New("Supplied Query param does not exists")
	}
	return parseQueryParam(q), nil
}

func parseQueryParam(val string) interface{} {
	if !(strings.HasPrefix(val, "[") &&
		strings.HasSuffix(val, "]")) {
		return val
	}
	r := strings.NewReplacer("[", "", "]", "")
	replaced := r.Replace(val)
