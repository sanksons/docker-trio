...
func (ss StandardSizeErrorResponse) Execute(io workflow.WorkFlowData) (workflow.WorkFlowData, error) {
	profiler := logger.NewProfiler()
	logger.StartProfile(profiler, common.POST_ERROR)
	defer func() {
		logger.EndProfile(profiler, common.POST_ERROR)
	}()
	res, _ := io.IOData.Get(common.STANDARDSIZE_ERROR)
