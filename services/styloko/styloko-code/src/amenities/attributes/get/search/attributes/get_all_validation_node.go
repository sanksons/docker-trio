...
func (d QueryParamDecision) GetDecision(io workflow.WorkFlowData) (bool, error) {
	...
	logger.StartProfile(profiler, QUERY_DECISION)
	defer func() {
		logger.EndProfile(profiler, QUERY_DECISION)
	}()
	io.ExecContext.Set(florest_constants.MONITOR_CUSTOM_METRIC_PREFIX, CUSTOM_ATTRIBUTE_GET_ALL)
