...
func TestWorkflowECInmemoryImplGetSet(t *testing.T) {
	...
	t1err := testWorkflowECInstance.Set(TEST_KEY_1, TEST_VALUE_1)
	if t1err != nil {
		t.Error("Failed to Set Workflow Execution Context Key")
	}
	t2err := testWorkflowECInstance.Set(TEST_KEY_2, TEST_VALUE_2)
	if t2err != nil {
		t.Error("Failed to Set Workflow Execution Context Key")
	}

	_, t1verr := testWorkflowECInstance.Get(TEST_KEY_1)
	if t1verr != nil {
		t.Error("Failed to Get Workflow Execution Context Key")
	}
	_, t2verr := testWorkflowECInstance.Get(TEST_KEY_2)
	if t2verr != nil {
		t.Error("Failed to Get Workflow Execution Context Key")
	}
}
...
func TestWorkflowECInmemoryImplGetSetValue(t *testing.T) {
	...
	value, t1verr := testWorkflowECInstance.Get(TEST_KEY_1)
	if t1verr != nil {
		t.Error("Failed to Get Workflow Execution Context Key")
	}
