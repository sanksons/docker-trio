...
func (ec *WorkFlowECInMemoryImpl) SetBuckets(bucket_id_map map[string]string) (err error) {
	return ec.Set(buckets, bucket_id_map)
}

func (ec *WorkFlowECInMemoryImpl) GetBuckets() (bucket_id_map map[string]string, err error) {
	res, err := ec.Get(buckets)
	if v, ok := res.(map[string]string); ok {
		bucket_id_map = v
	}
	return bucket_id_map, err
}

func (ec *WorkFlowECInMemoryImpl) GetExecuteThreadID() (execute_thread_id string, err error) {
	res, err := ec.Get(threadId)
	if v, ok := res.(string); ok {
		execute_thread_id = v
	}
	return execute_thread_id, err
}
