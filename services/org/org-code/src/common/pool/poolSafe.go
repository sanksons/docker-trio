...
//map to hold jobname,timestamp with key hostname to be pushed to redis
//when worker runs to trace them
...
func (p Safe) worker(job func(interface{}) error, id string) {
	defer RecoverHandler(p.name)
	// hostname, _ := os.Hostname()
	// uid := hostname + "_" + p.name + "_" + id
	// client, errs := ResourceFactory.GetRedisDriver("POOL")
	// if errs != nil {
	// 	logger.Error(fmt.Sprintf("Error while getting redis driver:%s", errs.Error()))
	// }
	// if client != nil {
	// 	client.HSet(uid, p.name, time.Now().String())
	// }
